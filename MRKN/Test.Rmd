...
```{r asdate}
date=as.Date(climtrend$DATE) # command transforms the DATE field in climtrend table into an R-formatted date array that R knows how to do things with
...
ggplot(climtrend, aes(date, TMAX))+geom_line()+ labs(y="Daily Maximum Temperature (degrees F)", x="Date")

ggplot(climtrend, aes(date, TMIN))+geom_line()+ labs(y="Daily Minimum Temperature (degrees F)", x="Date")

ggplot(climtrend, aes(date, PRCP))+geom_line()+ labs(y="Daily Rainfall (in)", x="Date")

```

we need to check this: "Some values are "NA" in temperature and precipitation; these have been trimmed automatically by ggplot, but this may not be the case for all functions. In general, you'll want to check meta data to see how missing data is labelled: -999 or NA are common choices." 
